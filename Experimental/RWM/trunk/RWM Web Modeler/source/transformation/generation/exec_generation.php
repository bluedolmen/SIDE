...
$ch = curl_init($serverUrl."/../exec_transformation.php?idVersion=$idModel&idTransformation=".$_GET["idTransformation"]);
...
$sql = "SELECT m.mm_file as mm, g.generationMustBeDownloaded, t.fileName as templateModel, mimetype FROM generator g, metamodels m, templateModels t "
...
if ($result) {
	while ($row = mysql_fetch_assoc($result)) {
		...
		$cmd = "java -Dfile.encoding=ISO-8859-1 -classpath org.rwm.acceleo.jar:lib/fr.obeo.acceleo.template_3.0.0.200809122321.jar:lib/fr.obeo.acceleo.tools_3.0.0.200809122321.jar:lib/org.acceleo.engine.jar:lib/org.acceleo.engine_3.0.0.qualifier.jar:lib/org.acceleo.gen.jar:lib/org.acceleo.gen_3.0.0.qualifier.jar:lib/org.acceleo.parser.jar:lib/org.acceleo.parser_3.0.0.qualifier.jar:lib/org.acceleo.tools.jar:lib/org.eclipse.emf.common_2.3.2.v200802051830.jar:lib/org.eclipse.emf.ecore.xmi_2.3.2.v200802051830.jar:lib/org.eclipse.emf.ecore_2.3.2.v200802051830.jar org.rwm.m2t.Main ../metamodels/$templateModel ".str_replace(" ","\ ",$tmp_out_model_name)." ../metamodels/$mm ".str_replace(" ","\ ",$tmp_out_directory);
		exec($cmd);
	}
}
...
$result = mysql_query($sql);
while ($row = mysql_fetch_assoc($result)) {
	$cmd = "ant -Dgenerated_dir=\"".realpath($tmp_out_directory)."\" -f ../metamodels/".$row["fileName"];
	exec($cmd);
}
