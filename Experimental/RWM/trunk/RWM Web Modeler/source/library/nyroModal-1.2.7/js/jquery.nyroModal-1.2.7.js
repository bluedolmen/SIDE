...
jQuery(function($) {
	...
	function fileType() {
		...
		if (from) {
			...
			var hashLoc = getHash(this.location.href);
			var curLoc = this.location.href.substring(0, this.location.href.length - hashLoc.length);
			var req = from.href.substring(0, from.href.length - hash.length);
	...
	function fillContent() {
		...
		modal.content.html(modal.tmp.contents()); //contents());
		modal.tmp.empty();
		wrapContent();
	...
	function filterScript(data) {
		...
		$.each($.clean({0:data}, this.ownerDocument), function() {
			if (!($.nodeName(this, "script") && this.src && $(this).attr('rel') != 'forceLoad'))
				tmp.push(this);
		});
		return tmp;
	}
