...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="d:type">
		...
		<xsl:apply-templates
			select="d:associations/d:child-association|d:association">
	...
	<xsl:template match="d:aspect">
		<xsl:element name="aspectSet">
			<xsl:attribute name="name"><xsl:value-of
				select="substring-after(text(),':')" /></xsl:attribute>
			<xsl:attribute name="title"><xsl:value-of select="d:title/text()" /></xsl:attribute>
			<xsl:attribute name="description"><xsl:value-of
				select="d:description/text()" /></xsl:attribute>
			<xsl:apply-templates select="d:properties/d:property" />
		...
		<xsl:element name="associationSet">
			...
			<xsl:element name="firstEnd">
				...
				<xsl:attribute name="linkedClass">//@classSet.<xsl:number
					value="$source_position" format="1"></xsl:number></xsl:attribute>
			</xsl:element>
