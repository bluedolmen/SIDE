...
<xsl:stylesheet version="2.0"
	...
	xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:MMUseCase="http://MMUseCase"
	xmlns:xmi="http://www.omg.org/XMI" xmlns:d="http://www.alfresco.org/model/dictionary/1.0">
			...
			<xsl:for-each select="d:model">
				<xsl:element name="packageSet">
					...
					<xsl:attribute name="name"><xsl:value-of
						select="substring-before(@name,':')" /></xsl:attribute>
		...
		<xsl:element name="classSet">
			...
			<xsl:attribute name="associations">
				<xsl:for-each select="d:associations/d:child-association">
					...
					<xsl:value-of
				select="concat(generate-id(//d:child-association[@name=$name]),' ')" />					
				</xsl:for-each>
				<xsl:for-each select="d:associations/d:association">
					<xsl:variable name="name"><xsl:value-of select="@name" /></xsl:variable>
					<xsl:value-of
				select="concat(generate-id(//d:association[@name=$name]),' ')" />					
				</xsl:for-each>
			</xsl:attribute>
	...
	<xsl:template match="d:parent">
		<xsl:variable name="prefix">
			<xsl:value-of select="substring-before(ancestor::d:model/@name,':')" />
		</xsl:variable>
		...
		<generalizations xmi:id="{generate-id(.)}">
			<xsl:attribute name="Target"><xsl:value-of
				select="generate-id(//d:type[@name=$name])" /></xsl:attribute>
		</generalizations>
	</xsl:template>
	...
	<xsl:template match="d:child-association|d:association">
		<xsl:element name="associationSet">
			...
			<xsl:attribute name="source"><xsl:value-of
				select="generate-id(parent::d:associations/parent::d:aspect)" /><xsl:value-of
				select="generate-id(parent::d:associations/parent::d:type)" /></xsl:attribute>
			<xsl:variable name="target">
				<xsl:value-of select="d:target/d:class/text()" />
			</xsl:variable>
			<xsl:attribute name="destination"><xsl:value-of
				select="generate-id(//d:type[@name=$target])" /></xsl:attribute>
