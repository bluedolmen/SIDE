...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="d:type">
		<xsl:element name="classSet">
			<xsl:attribute name="documentation"><xsl:value-of
				select="d:documentation/text()" /></xsl:attribute>
		...
		<xsl:choose>
			<xsl:when test="//d:types/d:type/@name=$parent_name">
				<xsl:element name="generalizations">
					<xsl:attribute name="href">#//@classSet.<xsl:number
			...
			<xsl:otherwise>
				<xsl:element name="hasComments">
					<xsl:attribute name="value">This class inherits from <xsl:value-of
						select="$parent_name" /> wich is in another Alfresco model.</xsl:attribute>
				</xsl:element>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="add_aspects_attribute">
		<xsl:if test="//d:aspects/d:aspect/@name=text()">
			...
			<xsl:variable name="aspect_link">
				//@aspectSet.
				<xsl:value-of select="$aspect_position" />
			</xsl:variable>
	...
	<xsl:template match="d:child-association|d:association">
		...
		<xsl:element name="associationSet">
			...
			<xsl:element name="firstEnd">
				...
				<xsl:attribute name="linkedClass">//@<xsl:value-of
					select="$source_type"></xsl:value-of>.<xsl:number value="$source_position"
					format="1"></xsl:number></xsl:attribute>
			</xsl:element>
			<xsl:element name="secondEnd">
				...
				<xsl:choose>
					<xsl:when test="//d:types/d:type/@name=$target_name">
						<xsl:attribute name="linkedClass">//@classSet.<xsl:number
							value="position()-1" format="1"></xsl:number></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="linkedClass" />
						<xsl:element name="hasComments">
							<xsl:attribute name="value">This association has <xsl:value-of
								select="$target_name" /> as target wich depends of another Alfresco model.</xsl:attribute>
						</xsl:element>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:element>
