		...
		for (Iterator iter = artifacts.iterator(); iter.hasNext();) {
			...
			if (!artifact.isOptional() && filter.include(artifact)) {
				try {
					...
					context.getLog().info("--------------------------------------------------------------");
					context.getLog().info("DEBUG い = artifact groupId="+artifact.getGroupId());
					context.getLog().info("DEBUG い = artifact artifactId="+artifact.getArtifactId());
					context.getLog().info("DEBUG い = artifact Version="+artifact.getVersion());
					context.getLog().info("DEBUG い = artifact Type="+type);
					context.getLog().info("DEBUG い = artifact Scope="+artifact.getScope());
					context.getLog().info("--------------------------------------------------------------");
					if ("tld".equals(type)) {
						...
						if (artifact.getScope().equals(Artifact.SCOPE_PROVIDED)) {
							context.getLog().info("DEBUG い = Artifact Scope="+Artifact.SCOPE_PROVIDED+",so associated ressource will not included");
						} else {
							copyFile(id, context, artifact.getFile(), LIB_PATH + targetFileName);
							context.getLog().info("DEBUG い = Artifact adding  " + artifact.getId() + " to AMP in " + LIB_PATH);
						}
