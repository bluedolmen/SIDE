...
public class Builder {
	static final String mavenNS = "http://maven.apache.org/POM/4.0.0";
	...
	private static void writePomFile(File pomFile, List<ModuleConstraint> res) throws Exception, JDOMException, FileNotFoundException, IOException {
		...
		if (deps == null) {
			deps = createMavenElement("dependencies");
			pom.getRootElement().addContent(deps);
		}
		// add dependencies
		for (ModuleConstraint moduleConstraint : res) {
			Element dep = createMavenElement("dependency");

			Element groupId = createMavenElement("groupId").setText(moduleConstraint.getGroupId());
			Element artifactId = createMavenElement("artifactId").setText(moduleConstraint.getArtifactId());
			Element version = createMavenElement("version").setText(moduleConstraint.getVersionRange());
			Element type = createMavenElement("type").setText(moduleConstraint.getModuleType());
			Element scope = createMavenElement("scope").setText("compile");
		...
		Element artifactId = getMavenChild(pom.getRootElement(), "artifactId");
		...
		for (File file : lPlug) {
			...
			try {
				...
				for (Object object : lmd) {
					...
					String id = getAttributeValue(el, "moduleId");
					String tech_version = getAttributeValue(el, "technologyVersion");
					String moduleType = getAttributeValue(el, "moduleType");
					String versionNumMin = getAttributeValue(el, "versionMin");
					String versionNumMax = getAttributeValue(el, "versionMax");
	...
	public static Element createMavenElement(String tag) {
		return new Element(tag, mavenNS);
	}

	public static Element getMavenChild(Element parent, String tag) {
		return parent.getChild(tag, Namespace.getNamespace(mavenNS));
	}

	public static String getAttributeValue(Element node, String attributeName) {
		return node.getAttributeValue(attributeName, Namespace.getNamespace(mavenNS));
	}
}
