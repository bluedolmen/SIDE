...
<%script type="Page" name="facetmapfacetsbasicfacets"%>
<%if (eContainer() != null) {%>
<%ID.toLowerCase().nPut("basicfacets_name")%>
<%for (portlets){%>
...
<%getProperty("facetmap.webapps")%>facetmap-facets-<%nGet("basicfacets_name").substring(nGet("basicfacets_name").lastIndexOf("-") +1)%>/xsl/display/includes/basic-Facets.xsl
<%}%>
<%}%>
<%}%>
<%}%>
<%}%>
<%}%>
<%script type="Page" name="facetmapGenerator" file="<%facetmapfacetsbasicfacets%>" post="trim()"%>
<%if (eContainer() != null) {%>
<%ID.toLowerCase().nPut("basicfacets_name")%>
<%for (portlets){%>
<%for (associationPortlet){%>
...
version="1.0">
  ...
  <xsl:param name="app">facetmap-content-<%nGet("basicfacets_name").substring(nGet("basicfacets_name").lastIndexOf("-") +1)%></xsl:param>
  <xsl:param name="app2">facetmap-facets-<%nGet("basicfacets_name").substring(nGet("basicfacets_name").lastIndexOf("-") +1)%></xsl:param>
