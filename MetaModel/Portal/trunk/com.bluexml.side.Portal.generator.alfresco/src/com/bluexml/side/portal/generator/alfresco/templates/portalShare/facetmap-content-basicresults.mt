...
%>
<%script type="Page" name="facetmapcontentbasicresults"%>
<%if (eContainer() != null) {%>
<%ID.toLowerCase().nPut("bresults_name")%>
<%for (portlets){%>
...
<%getProperty("facetmap.webapps")%>facetmap-content-<%nGet("bresults_name").substring(nGet("bresults_name").lastIndexOf("-") +1)%>/xsl/display/includes/basic-Results.xsl
<%}%>
<%}%>
<%}%>
<%}%>
<%}%>
<%}%>
<%script type="Page" name="facetmapGenerator" file="<%facetmapcontentbasicresults%>" post="trim()"%>
<%if (eContainer() != null) {%>
