...
<%for (isPortletInternal.form.forms.filter("form.FormClass")){%>
   <content mimetype="" icon="<%filter("form.ClassReference").real_class.getPrefixedQName("-")%>" label="menu.create-content.<%real_class.name%>" itemid="<%real_class.getPrefixedQName()%>"<%if (eContainer("form.FormCollection").name != "default"){%> formid="<%eContainer("form.FormCollection").name%>"<%}%> isContainer="<%real_class.isFolder()%>"/>
<%}%>
</create-content>
<%}%>
