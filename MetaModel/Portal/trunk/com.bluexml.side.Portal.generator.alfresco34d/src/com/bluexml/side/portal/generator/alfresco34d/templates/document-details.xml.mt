...
<%if (pageSet[ID.toLowerCase().trim() == "document-details"].nSize() > 0) {%><%getProperty("alf.share.paths.core.site-data")%>/template-instances/document-details.xml<%}%>

<%script type="Portal" name="alfrescoGenerator" file="<%validatedFilename%>"%>
<%for (pageSet[ID.toLowerCase().trim() == "document-details"].portlets.associationPortlet){%>
