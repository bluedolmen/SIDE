...
<data-type name="<%getPrefixedQName()%>">
   <analyser-class><%if (metainfo[key.equalsIgnoreCase("analyser")].nSize() > 0){%><%metainfo[key.equalsIgnoreCase("analyser")].nFirst().value%><%}else{%>org.alfresco.repo.search.impl.lucene.analysis.AlfrescoStandardAnalyser<%}%></analyser-class>
