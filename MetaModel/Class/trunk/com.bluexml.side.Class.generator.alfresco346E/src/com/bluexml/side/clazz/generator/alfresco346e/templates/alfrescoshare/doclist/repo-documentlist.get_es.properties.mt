...
<%if (eContainer() == null) {%><%getProperty("alf.share.paths.core.site-webscripts")%>org/alfresco/components/documentlibrary/repo-documentlist.get_es.properties<%}%>
...
details.created.on=Creado:
details.created.by=Creado por:
details.modified.on=Modificado:
details.modified.by=Modificado por:
details.editing-started.on=Descargado y bloqueado:
details.editing-started.by=Descargado y bloqueado por:
details.by=Por:
...
details.categories=Categor\u00edas:
details.categories.none=(Ninguna)
...
details.banner.locked=Este documento est\u00e1 bloqueado por {0}.
...
tip.favourite-document.remove=Eliminar documento de favoritos
tip.favourite-folder.add=A\u00f1adir carpeta a favoritos
tip.favourite-folder.remove=Eliminar carpeta de favoritos
...
message.favourite.failure=No se pudo actualizar la lista de favoritos
...
message.simple-workflow.failed=La acci\u00f3n del flujo de trabajo no se puede completar.
...
edit-details.label.edit-metadata=P\u00e1gina de edici\u00f3n de metadatos completa\u0085
...
# SIDE sort extension
label.sortby=Clasificar por: Nombre
