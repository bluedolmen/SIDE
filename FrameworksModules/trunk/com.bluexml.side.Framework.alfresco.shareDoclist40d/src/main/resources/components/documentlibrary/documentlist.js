...
(function() {
	 ...
	 * YUI Library aliases
	 */
	var Dom = YAHOO.util.Dom, Event = YAHOO.util.Event, DDM = YAHOO.util.DragDropMgr, JSON = YAHOO.lang.JSON;

	/**
	 * Alfresco Slingshot aliases
	 */
	var $html = Alfresco.util.encodeHTML, $links = Alfresco.util.activateLinks, $combine = Alfresco.util.combinePaths, $userProfile = Alfresco.util.userProfileLink, $siteURL = Alfresco.util.siteURL, $date = function $date(
			date, format) {
	 ...
	 * Preferences
	 */
	var PREFERENCES_DOCLIST = "org.alfresco.share.documentList", PREF_SORT_ASCENDING = PREFERENCES_DOCLIST + ".sortAscending", PREF_SORT_FIELD = PREFERENCES_DOCLIST + ".sortField", PREF_SHOW_FOLDERS = PREFERENCES_DOCLIST
			+ ".showFolders", PREF_SIMPLE_VIEW = PREFERENCES_DOCLIST + ".simpleView";
	...
	YAHOO.extend(Alfresco.DnD, YAHOO.util.DDProxy, {
		...
		/**
		 * Handles the beginning of a drag operation by setting up the proxy
		 * image element.
		 */
		 ...
		 * asynchronous it is not know if the operation has been a success at
		 * the time this function is invoked so it uses the _inFlight variable
		 * to check whether or not a valid drop target was used.
		 * 
		 * @param The
		 *            event object
		 */
		...
		/**
		 * Animates an object to move it to the location of a target object.
		 * This should typically be animating the proxy object to return to its
		 * source.
		 * 
		...
		/**
		 * Handles a drop operation by determining whether or not a valid drop
		 * has been performed (e.g. a document or folder onto a folder - NOT a
		...
		onDragDrop : function DL_DND_onDragDrop(e, id) {
			...
			if (DDM.interactionInfo.drop.length > 0) {
				...
				if (Dom.isAncestor(this.docLib.widgets.dataTable.getContainerEl(), dropTarget)) {
					// If the drop target is contained within the data table
					// then process "normally"...
					var targetRecord = this.docLib.widgets.dataTable.getRecord(Dom.get(id)), targetNode = targetRecord.getData();

					if (targetNode.node.isContainer) {
						// Indicate that a request is about to be made - this
						// will prevent the endDrag
						// function from animating the proxy to return to its
						// source...
						this._inFlight = true;
					...
					// The "documentDroppable" class is not defined in any CSS
					// files but is simply used as
					// a marker to indicate that the element can be used as a
					// document drop target. Only
					// documents are dragged and dropped onto these elements
					// should result in the drop
					// target request being fired (it's possible that an element
					// could be specified as a
					// YUI drag and drop target for the purposes of controlling
					// drag events without actually
		...
		_performMove : function DL_DND__performMove(nodeRef, path) {
			...
			var toMoveRecord = this.docLib.widgets.dataTable.getRecord(this.getEl()), webscriptName = "move-to/node/{nodeRef}", multipleFiles = [];

			multipleFiles.push(this.getEl().id);

			// Success callback function:
			// If the operation succeeded then update the tree and refresh the
			// document list.
			var fnSuccess = function DLCMT__onOK_success(p_data) {
				this._inFlight = false; // Indicate that a request is no longer
										// "in-flight"

				var result, successCount = p_data.json.successCount, failureCount = p_data.json.failureCount;
			...
			// destination: targetNode.location.path + "/" +
			// targetNode.location.file

			// Failure callback function:
			// If the move operation has failed then animate the proxy to return
			// it to the
		...
		/**
		 * The id of the current window timeout. This should only be non-null if
		 * a proxy has been dropped onto a valid drop target that was NOT part
		 * of the DocumentList DataTable widget. This id is used to clear the
		 * current timeout associated with a drop if the target owner responds
		 * with the node ref.
		 * 
		 ...
		 * drop target that is NOT part of the DocumentList DataTable widget.
		 * This clears the timeout, resets the timeout id to null and removes
		 * the inflight status of the drop operation.
		 * 
		...
		_setFailureTimeout : function DL_DND__setFailureTimeout() {
			...
			this._currTimeoutId = window.setTimeout(function() {
				// An attempt was made to drop a document or folder into a
		...
		/**
		 * If the element the proxy has been dragged over is a folder, then
		 * style class indicating that it is a viable drop target is added.
		 * 
		 * @param e
		 *            The event object
		 * @param id
		...
		/**
		 * If the element the proxy has been dragged out of is a folder, then
		 * style class indicating that it is a viable drop target is removed.
		 * 
		 * @param e
		 *            The event object
		 * @param id
		 *            The id of the element that the proxy has been dragged out
		 *            of
		 */
	 ...
	 * @param filter
	 *            {object} Object literal containing new filter parameters
	 ...
	 * @param locn
	 *            {object} Location object containing path and folder name to
	 *            navigate to
	 * @return {string} Mark-up for use in node attribute
	 */
	 ...
	 * @param category
	 *            {Array} category[0] is name, category[1] is qnamePath
	 * @return {string} Mark-up for use in node attribute
	 */
	...
	Alfresco.DocumentList.generateFileFolderLinkMarkup = function DL_generateFileFolderLinkMarkup(scope, record) {
		var jsNode = record.jsNode, recordSiteName = $isValueSet(record.location.site) ? record.location.site.name : null, html;

		// Test for: handling a link and the link's target does not live within
		// this site
		if (jsNode.isLink && recordSiteName !== scope.options.siteId) {
			if (jsNode.isContainer) {
				// Create a URL to either the Repository or Site document
				// library where the link target lives
	...
	Alfresco.DocumentList.generateThumbnailUrl = function DL_generateThumbnailUrl(record) {
		var jsNode = record.jsNode, nodeRef = jsNode.isLink ? jsNode.linkedNode.nodeRef : jsNode.nodeRef;
	...
	Alfresco.DocumentList.generateFavourite = function DL_generateFavourite(scope, record) {
		var jsNode = record.jsNode, i18n = "favourite." + (jsNode.isContainer ? "folder." : "document."), html = "";
	...
	Alfresco.DocumentList.generateLikes = function DL_generateLikes(scope, record) {
		var node = record.node, likes = record.likes, i18n = "like." + (node.isContainer ? "folder." : "document."), html = "";
	...
	Alfresco.DocumentList.generateComments = function DL_generateComments(scope, record) {
		var node = record.node, actionUrls = scope.getActionUrls(record), url = actionUrls[node.isContainer ? "folderDetailsUrl" : "documentDetailsUrl"] + "#comment", i18n = "comment."
				+ (node.isContainer ? "folder." : "document.");
	 ...
	 *            {object} DocumentLibrary instance
	 * @param oUser
	 *            {object} Object literal container user data
	...
	YAHOO.lang
			.augmentObject(
					Alfresco.DocumentList.prototype,
					{
						...
						options : {
							/**
							 * Flag indicating whether sort direction is
							...
							/**
							 * Flag indicating whether folders are visible or
							 * not.
							 * 
							...
							/**
							 * Flag indicating whether the list shows a detailed
							 * view or a simple one.
							 * 
							 * @property simpleView
							 * @type boolean
							 */
							...
							/**
							 * Current siteId. Not used in Repository working
							 * mode.
							 * 
							 * @property siteId
							 * @type string
							...
							/**
							 * ContainerId representing root container. Not used
							 * in Repository working mode.
							 * 
							...
							/**
							 * FileName to highlight on initial DataTable
							 * render.
							 * 
							...
							/**
							 * Number of multi-file uploads before grouping the
							 * Activity Post
							 * 
							...
							/**
							 * Valid inline edit mimetypes Currently allowed are
							 * plain text, HTML and XML only
							 * 
							...
							/**
							 * Flag to indicate the list may be updated as a
							 * results of a REST API call
							 * 
							...
							/**
							 * Whether the user viewing the document library is
							 * a manager of the site.
							 * 
						...
						state : {
							/**
							 * True if an an edit offline ajax call is in
							 * process
							 * 
						...
						/**
						 * Object literal of selected states for visible files
						 * (indexed by nodeRef).
						 * 
						...
						/**
						 * Deferred function calls for after a document list
						 * update
						 * 
						...
						/**
						 * Dynamic controls that take part in the
						 * deactivateDynamicControls event
						 * 
						...
						/**
						 * Indicates whether or not we allow the HTML5 drag and
						 * drop capability
						 * 
						...
						/**
						 * Indicates whether or not the browser supports the
						...
						/**
						 * Drag and drop dragEnter / dragLeave event reference
						 * counter
						 * 
						...
						/**
						 * Tracks currently highlighted folder when dragging
						 * files
						 * 
						...
						/**
						 * Registered metadata renderers. Register new renderers
						 * via registerRenderer() or "registerRenderer" bubbling
						 * event
						 * 
						 * @property renderers
						 * @type object
						...
						onReady : function DL_onReady() {
							...
							this.widgets.previewTooltip.contextTriggerEvent.subscribe(function(type, args) {
								var context = args[0], oRecord = me.widgets.dataTable.getRecord(context.id), record = oRecord.getData();
							...
							try {
								...
								/*
								 * The only exception that gets thrown here is
								 * when the browser is not supported (Opera, or
								 * not A-grade)
								 */
						 ...
						 * @protected
						 * @param response
						 *            {object} AJAX response
						 * @param record
						 *            {YAHOO.widget.Record} Record for the item
						 *            being edited
						 */
						_insituCallback : function DL__insituCallback(response, record) {
							// Reload the node's metadata
							var jsNode = record.jsNode, nodeRef = jsNode.nodeRef;
							...
							Alfresco.util.Ajax.request({
								...
								successCallback : {
									fn : function DL_insituCallback_refreshSuccess(response) {
										...
										// Prevent hide call which briefly shows
										// stale data
										return false;
									},
									scope : this
						...
						fnRenderCellSelected : function DL_fnRenderCellSelected() {
							...
							return function DL_renderCellSelected(elCell, oRecord, oColumn, oData) {
								...
								var jsNode = oRecord.getData("jsNode"), nodeRef = jsNode.nodeRef;

								elCell.innerHTML = '<input id="checkbox-' + oRecord.getId() + '" type="checkbox" name="fileChecked" value="' + nodeRef + '"'
										+ (scope.selectedFiles[nodeRef] ? ' checked="checked">' : '>');
							};
						},
						...
						fnRenderCellStatus : function DL_fnRenderCellStatus() {
							...
							return function DL_renderCellStatus(elCell, oRecord, oColumn, oData) {
								...
								var record = oRecord.getData(), node = record.jsNode, indicators = record.indicators, indicator, label, desc = "";

								if (indicators && indicators.length > 0) {
									for ( var i = 0, ii = indicators.length; i < ii; i++) {
										indicator = indicators[i];
										// Note: deliberate bypass of
										// scope.msg() function
										label = Alfresco.util.message(indicator.label, scope.name, indicator.labelParams);
										/*
										 * label = YAHOO.lang.substitute(label,
										 * record, function
										 * DL_renderCellStatus_label(p_key,
										 * p_value, p_meta) { return
										 * Alfresco.util.findValueByDotNotation(record,
										 * p_key); });
										 */
										label = Alfresco.util.substituteDotNotation(label, record);

										desc += '<div class="status"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/indicators/' + indicator.icon + '" title="' + label
												+ '" alt="' + indicator.id + '" /></div>';
									}
								}
						...
						fnRenderCellThumbnail : function DL_fnRenderCellThumbnail() {
							...
							return function DL_renderCellThumbnail(elCell, oRecord, oColumn, oData) {
								...
								var containerTarget; // This will only get
														// set if thumbnail
														// represents a
														// container
								...
								if (scope.options.simpleView) {
									...
									if (isContainer) {
										elCell.innerHTML = '<span class="folder-small">' + (isLink ? '<span class="link"></span>' : '')
												+ (scope.dragAndDropEnabled ? '<span class="droppable"></span>' : '') + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, record) + '<img id="'
												+ imgId + '" src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/folder-32.png" /></a>';
										containerTarget = new YAHOO.util.DDTarget(imgId); // Make
																							// the
																							// folder
																							// a
																							// target
										...
										elCell.innerHTML = '<span id="' + id + '" class="icon32">' + (isLink ? '<span class="link"></span>' : '')
												...
												+ 'components/images/filetypes/' + Alfresco.util.getFileIcon(name) + '" alt="' + extn + '" title="' + $html(name) + '" /></a></span>';
						...
						fnRenderCellDescription : function DL_fnRenderCellDescription() {
							...
							return function DL_renderCellDescription(elCell, oRecord, oColumn, oData) {
								...
								if (($isValueSet(properties.lockOwner) || $isValueSet(properties.workingCopyOwner)) && !jsNode.hasAspect("trx:transferred")) {
									var bannerUser = properties.lockOwner || properties.workingCopyOwner, bannerLink = Alfresco.DocumentList.generateUserLink(scope, bannerUser);

									/* Google Docs Integration */
									if (record.workingCopy) {
										if ($isValueSet(record.workingCopy.googleDocUrl)) {
											if (bannerUser.userName === Alfresco.constants.USERNAME) {
												desc += '<div class="info-banner">'
														+ scope.msg("details.banner.google-docs-owner", '<a href="' + record.workingCopy.googleDocUrl + '" target="_blank">'
																+ scope.msg("details.banner.google-docs.link") + '</a>') + '</div>';
											} else {
												desc += '<div class="info-banner">'
														+ scope.msg("details.banner.google-docs-locked", bannerLink, '<a href="' + record.workingCopy.googleDocUrl + '" target="_blank">'
																+ scope.msg("details.banner.google-docs.link") + '</a>') + '</div>';
											}
										} else {
								...
								if (metadataTemplate && YAHOO.lang.isArray(metadataTemplate.lines)) {
									var fnRenderTemplate = function fnRenderTemplate_substitute(p_key, p_value, p_meta) {
										...
										// render value from properties or
										// custom renderer
						 ...
						 * @param args
						 *            {array} Event parameters (property name,
						 *            rendering function)
						 */
						 ...
						 * @param propertyName
						 *            {string} Property name to attach this
						 *            renderer to
						 * @param renderer
						...
						_setupMetadataRenderers : function DL__setupMetadataRenderers() {
							...
							this.registerRenderer("date", function(record, label) {
								...
								html = '<span class="item">' + label
										+ this.msg("details." + dateI18N + "-by", $relTime(dateProperty), Alfresco.DocumentList.generateUserLink(this, properties.modifier)) + '</span>';
							...
							this.registerRenderer("description", function(record, label) {
								var jsNode = record.jsNode, properties = jsNode.properties, id = Alfresco.util.generateDomId(), html = '<span id="' + id + '" class="faded">' + label
							...
							this.registerRenderer("tags", function(record, label) {
								var jsNode = record.jsNode, properties = jsNode.properties, id = Alfresco.util.generateDomId(), html = "";
								...
								if (jsNode.hasAspect("cm:generalclassifiable")) {
									...
									if (categories.length > 0) {
										for ( var i = 0, j = categories.length; i < j; i++) {
											...
											html += '<span class="category"><a href="#" class="filter-change" rel="' + Alfresco.DocumentList.generateCategoryMarkup(category) + '">'
													+ $html(category[0]) + '</a></span>' + (j - i > 1 ? ", " : "");
										}
									} else {
						...
						_setupDataSource : function DL__setupDataSource() {
							...
							this.widgets.dataSource = new YAHOO.util.DataSource($combine(this.dataSourceUrl), {
								...
								responseSchema : {
									...
									metaFields : {
										...
										totalRecordsUpper : "totalRecordsUpper" // if
																				// null
																				// then
																				// totalRecords
																				// is
							...
							this.widgets.dataSource.doBeforeCallback = function DL_doBeforeCallback(oRequest, oFullResponse, oParsedResponse) {
								...
								// Check for parent node - won't be one for
								// multi-parent queries (e.g. tags)
								...
								// Work out the current status of the document
								// list (this will be used to determine what
								// user assistance
								// is provided if the doc list is empty, or
								// appears as empty)...
								var itemCounts = me.doclistMetadata.itemCounts, empty = (itemCounts.documents === 0 && itemCounts.folders === 0), hiddenFolders = (itemCounts.documents === 0
										&& !me.options.showFolders && itemCounts.folders > 0);

								// Define a re-usable function for seting IDs...
								// Get the children of the supplied node and
								// append "-instance" to any child nodes that
								// have an
								// "id" attribute in the template. This ensures
								// that the clone has a unique ID within the
								// page and can be accurately targeted later
								// (i.e. to add event listeners to).
								...
								// In documentlist.lib.ftl there are a number of
								// DOM structures that are not displayed, these
								// can
								// cloned to display the relevant information to
								// the user based on content, display options,
								// site
								// ownership and access rights. All of theses
								// DOM "snippets" need to be added to a main
								// container
								// which controls the overall display (of
								// borders, etc).
								var template = Dom.get(me.id + "-main-template"), main = template.cloneNode(true), container = Dom.getFirstChild(main), templateInstance = null, elements = null;
								...
								if (permissions) {
									...
									// Work out what to display based on the
									// boolean values calculated earlier...
									if (empty && !userCanUpload) {
										// If folder is empty, there are no
										// hidden folders and the user cannot
										// upload, then show the no items
										// info...
										...
										// ...or, if there are hidden subfolders
										// then show the option to reveal
										// them...
										...
										// ...or, if the folder is empty, there
										// are no hidden folders, the user can
										// upload AND the browser supports
										// the DND process, show the HTML5 DND
										// instructions...
										...
										// ...but if the folder is empty, there
										// are no hidden folders, the user can
										// upload BUT the browser does
										// NOT support the DND process then just
										// show the standard upload
										// instructions...
									...
									if (empty && userCanUpload && me.dragAndDropEnabled) {
										...
										if (empty && userCanUpload) {
											...
											// Show the New Folder other options
											// node...
									...
									// boolean values calculated earlier...
									if (hiddenFolders) {
										// If there are hidden subfolders then
										// show the option to reveal them...
										template = Dom.get(me.id + "-hidden-subfolders-template");
										templateInstance = template.cloneNode(true);
										Dom.removeClass(templateInstance, "hidden");
										updateIDs(templateInstance);
								...
								// Add a node in with a style of "clear" set to
								// both to ensure that the main div is given
								...
								// Finally set the innerHTML of the main node as
								// the text string of the YUI datatable
						...
						_setupDataTable : function DL__setupDataTable() {
							...
							// Update totalRecords on the fly with value from
							// server
							...
							// Hook tableMsgShowEvent to clear out fixed-pixel
							// width on <table> element (breaks resizer)
							this.widgets.dataTable.subscribe("tableMsgShowEvent", function(oArgs) {
								// NOTE: Scope needs to be DataTable
								this._elMsgTbody.parentNode.style.width = "";

								// Check to see whether or not the message is
								// the HTML5 drag and drop instructions. This
								// will be set if there are no documents or
								// folders in the displayed location. If the
								// HTML5
								// drag and drop instructions are shown then we
								// want to attach events to the "upload" and
								// "share" images...
								if (oArgs.html && oArgs.html.indexOf("docListInstructions") !== -1) {
									...
									if (toolbar !== null) {
										// This code is reliant upon the
										// template in documentlist.lib.ftl not
										// being changed.
										// It allows us to attach events to the
										// image and anchor, but not the
										// remaining text.
									...
									// See if the document list contains
									// instructions on showing hidden
									// sub-folders and if
									// it does, attach the appropriate events to
									// the relevant nodes...
							...
							this.widgets.dataTable.doBeforeLoadData = function DL_doBeforeLoadData(sRequest, oResponse, oPayload) {
								...
								if (oResponse.error) {
									...
									// Add an Alfresco.util.Node instance to
									// each result
							...
							this.widgets.dataTable
									.subscribe(
											"renderEvent",
											function DL_renderEvent() {
												Alfresco.logger.debug("DataTable renderEvent");

												// IE6 fix for long filename
												// rendering issue
												...
												if (this.options.highlightFile) {
													// Add a delay to ensure the
													// table is fully in the DOM
							...
							// Feature detection for drag and drop support (by
							// not attempting to attach the drag events
							// to anything we can prevent attempted uploads
							// happening - this is particularly important
							// with IE8 which would otherwise render the
							// highlights, but not process the upload).
							if (this.dragAndDropEnabled) {
								// Disable drop for the document body (we're
								// then going to do a specific override on the
								// DocumentList nodes)
						...
						/**
						 * Removes HTML5 drag and drop listeners from the
						 * document list.
						 * 
						...
						_removeDragAndDrop : function DL__removeDragAndDrop() {
							if (this.dragAndDropEnabled) {
								// Make the entire DocumentList available for
								// dropping files for uploading onto.
								try {
									// Add listeners to the HTML5 drag and drop
									// events fired from the entire doc list
						...
						/**
						 * Adds HTML5 drag and drop listeners to the document
						 * list.
						 * 
						 * @method _addDragAndDrop
						 */
						...
						/**
						 * Fired when an object starts getting dragged. The
						 * event is swallowed because we only want to allow drag
						 * and drop events that begin outside the browser window
						 * (e.g. for files). This prevents users attempting to
						 * drag and drop the document and folder images as if
						 * they could re-arrange the document lib structure.
						 * 
						 * @param e
						...
						/**
						 * Fired when an object is dragged onto any node in the
						 * document body (unless the node has been explicitly
						 * overridden to invoke another function). Swallows the
						 * event.
						 * 
						 * @param e
						 *            {object} HTML5 drag and drop event
						...
						/**
						 * Fired when an object is dragged over any node in the
						 * document body (unless the node has been explicitly
						 * overridden to invoke another function). Updates the
						 * drag behaviour to indicate that drops are not allowed
						 * and then swallows the event.
						 * 
						 * @param e
						 *            {object} HTML5 drag and drop event
						...
						/**
						 * Fired when an object is dropped onto any node in the
						 * document body (unless the node has been explicitly
						 * overridden to invoke another function). Swallows the
						 * event to prevent default browser behaviour (i.e.
						...
						/**
						 * Fired when an object is dragged into the DocumentList
						 * DOM element and then again when dragged into a Folder
						...
						onDocumentListDragEnter : function DL_onDocumentListDragEnter(e) {
							// Providing that the drop target a <TR> (or a child
							// node of a <TR>) in the DocumentList data table
							// then a record
							// will be returned from this call. If nothing is
							// returned then we cannot proceed with the file
							// upload operation.
							var el = e.target, oRecord = this.widgets.dataTable.getRecord(el), column = this.widgets.dataTable.getColumn(el), containerEl = this.widgets.dataTable.getContainerEl(), node;
							...
							if (oRecord && column && (el.tagName == "IMG" || el.className == "droppable") && column.getKey() == "thumbnail") {
								...
								// Firefox is a bit buggy with it's enter/leave
								// event matching
						...
						onDocumentListDragOver : function DL_onDocumentListDragOver(e) {
							// Firefox 3.6 set effectAllowed = "move" for files,
							// however the "copy" effect is more accurate for
							// uploads
						...
						/**
						 * Fired when an object is dragged out of the
						 * DocumentList DOM element or the Folder icon image DOM
						 * element.
						 * 
						 * @param e
						 *            {object} HTML5 drag and drop event
						...
						onDocumentListDragLeave : function DL_onDocumentListDragLeave(e) {
							...
							// returned then we cannot proceed with the file
							// upload operation.
							var el = e.target, oRecord = this.widgets.dataTable.getRecord(el), oColumn = this.widgets.dataTable.getColumn(el), record = oRecord ? oRecord.getData() : null;
							...
							try {
								...
								if (e.dataTransfer.files !== undefined && e.dataTransfer.files !== null && e.dataTransfer.files.length > 0) {
									...
									if (continueWithUpload) {
										...
										var directory = this.currentPath, directoryName = directory.substring(directory.lastIndexOf("/") + 1), destination = this.doclistMetadata.parent ? this.doclistMetadata.parent.nodeRef
												: null;
										...
										if (oRecord !== null) {
											...
											if (oRecord && oColumn && (e.target.tagName == "IMG" || (e.target.tagName == "SPAN" && Dom.hasClass(e.target, "droppable"))) && record.node.isContainer
													&& oColumn.getKey() == "thumbnail") {
						...
						onHistoryManagerReady : function DL_onHistoryManagerReady() {
							// Check whether to show upload dialog after initial
							// load. Suppressed if HTML5 drag-and-drop is
							// available.
						...
						getSelectedFiles : function DL_getSelectedFiles() {
							var files = [], oRecordSet = this.widgets.dataTable.getRecordSet(), oRecord, node, i, j;
						...
						selectFiles : function DL_selectFiles(p_selectType) {
							var oRecordSet = this.widgets.dataTable.getRecordSet(), checks = YAHOO.util.Selector.query('input[type="checkbox"]', this.widgets.dataTable.getTbodyEl()), len = checks.length, oRecord, record, i, fnCheck;
						...
						onSortField : function DL_onSortField(sType, aArgs, p_obj) {
							...
							if (eventTarget) {
								...
								if (title === "true" || title === "false") {
									...
									// onSortAscending will fire the
									// metadatRefresh event
						...
						onEventHighlightRow : function DL_onEventHighlightRow(oArgs) {
							...
							// cell where we'll inject the actions
							...
							if (elActions && elActions.firstChild === null) {
								...
								var oRecord = this.widgets.dataTable.getRecord(oArgs.target.id), record = oRecord.getData(), jsNode = record.jsNode, actions = record.actions, actionsEl = document
										.createElement("div"), actionHTML = "", actionsSel;
								...
								if (actionsSel.length > splitAt + (this.options.simpleView ? 0 : 1)) {
									var moreContainer = Dom.get(this.id + "-moreActions").cloneNode(true), containerDivs = YAHOO.util.Selector.query("div", moreContainer);
									...
									// Now make action items three onwards
									// children of the 2nd DIV
						...
						onEventUnhighlightRow : function DL_onEventUnhighlightRow(oArgs) {
							...
							// showing, or a dialog mask is present
						 ...
						 * Disconnected event handlers for action event
						 * notification
						 */
						...
						onActionEditOffline : function DL_onActionEditOffline(record) {
							if (!this.state.actionEditOfflineActive) {
								...
								var nodeRef = record.jsNode.nodeRef, displayName = record.displayName;
									...
									success : {
										...
										callback : {
											fn : function DL_oAEO_success(data) {
												...
												// The filterChanged event
												// causes the DocList to update,
												// so we need to run these
						...
						onActionCheckoutToGoogleDocs : function DL_onActionCheckoutToGoogleDocs(record) {
							var displayName = record.displayName, nodeRef = record.jsNode.nodeRef, path = record.location.path, fileName = record.fileName;
								...
								success : {
									...
									callback : {
										fn : function DL_oACTGD_success(data) {
											...
											// The filterChanged event causes
											// the DocList to update, so we need
											// to run these functions afterwards
						 ...
						 *            file to be actioned upon
						 */
						onActionCheckinFromGoogleDocs : function DL_onActionCheckinFromGoogleDocs(record) {
							var displayName = record.displayName, nodeRef = record.jsNode.nodeRef, originalNodeRef = new Alfresco.util.NodeRef(record.workingCopy.sourceNodeRef), path = record.location.path;
						...
						onChangeFilter : function DL_onChangeFilter(layer, args) {
							...
							if ((obj !== null) && (obj.filterId !== null)) {
								...
								var filter = Alfresco.util.cleanBubblingObject(obj), strFilter = window.escape(obj.filterId)
										+ (typeof obj.filterData !== "undefined" ? "|" + window.escape(obj.filterData) : "");

								Alfresco.logger.debug("DL_onChangeFilter: ", filter);

								// Only allow drag and drop behaviour if the
								// filter is changed to an actual
								// path (if the filter is anything else such as
								// tags then there won't be a specific
								...
								if (obj.doclistFirstTimeNav) {
									...
									// Do we think the history state will
									// change?
						...
						/**
						 * Highlight file event handler Used when a component
						 * (including the DocList itself on loading) wants to
						 ...
						 *            {object} Event fired (unused)
						 * @param args
						 *            {array} Event parameters (filename to be
						 *            highlighted)
						 */
						onHighlightFile : function DL_onHighlightFile(layer, args) {
							...
							if ((obj !== null) && ($isValueSet(obj.fileName))) {
								...
								if (recordFound !== null) {
									// Scroll the record into view and highlight
									// it
									var el = this.widgets.dataTable.getTrEl(recordFound), yPos = Dom.getY(el);
						...
						/**
						 * Deactivate Dynamic Controls event handler Only
						 * deactivates specifically defined controls.
						 * 
						 ...
						 * (Re-)Activates controls taking part in dynamic
						 * deactivation
						 * 
						...
						onLikes : function DL_onLikes(row) {
							var oRecord = this.widgets.dataTable.getRecord(row), record = oRecord.getData(), nodeRef = record.jsNode.nodeRef, likes = record.likes;
						 ...
						 *            element (or child thereof)
						 * @param prefKey
						 *            {String} The preferences key
						 */
						onFavourite : function DL_onFavourite(row) {
							var oRecord = this.widgets.dataTable.getRecord(row), record = oRecord.getData(), node = record.node, nodeRef = node.nodeRef;
							...
							var responseConfig = {
								failureCallback : {
									fn : function DL_oFD_failure(event, p_oRow) {
										// Reset the flag to it's previous state
										var oRecord = this.widgets.dataTable.getRecord(p_oRow), record = oRecord.getData(), node = record.node;
							...
							var fnPref = record.isFavourite ? "add" : "remove", prefKey = node.isContainer ? Alfresco.service.Preferences.FAVOURITE_FOLDERS
									: Alfresco.service.Preferences.FAVOURITE_DOCUMENTS;
						...
						/**
						 * Updates document list by calling data webscript with
						 * current site and path
						 * 
						 * @method _updateDocList
						 * @param p_obj.filter
						 *            {object} Optional filter to navigate with
						 * @param p_obj.page
						 *            {string} Optional page to navigate to
						 *            (defaults to this.currentPage)
						 */
						_updateDocList : function DL__updateDocList(p_obj) {
							...
							}, p_obj.filter !== undefined ? p_obj.filter : this.currentFilter), successPage = p_obj.page !== undefined ? p_obj.page : this.currentPage, loadingMessage = null, timerShowLoadingMessage = null, me = this, params = {
							...
							var successHandler = function DL__uDL_successHandler(sRequest, oResponse, oPayload) {
								destroyLoaderMessage();
								// Updating the Doclist may change the file
								// selection
								...
								if (oResponse.status === 401) {
									...
									try {
										if (oResponse.status === 404) {
											// Folder not found (via the HTTP
											// "404 Not Found" response) -
											// deactivate dynamic controls only
											YAHOO.Bubbling.fire("deactivateDynamicControls");
										} else {
											// Site or container not found (e.g.
											// via the HTTP "410 Gone" response)
											// or more serious - deactivate all
											// controls
						...
						_buildDocListParams : function DL__buildDocListParams(p_obj) {
							...
							// Build the URI stem
							var uriPart = siteMode ? "site/{site}/{container}" : "node/alfresco/company/home", params = YAHOO.lang.substitute("{type}/" + uriPart
							...
							// Filter parameters
							// SIDE if options.search.type exists use SIDE metadata filter
							if (this.options.search.type && this.options.search.type.length > 0) {
								params += "?filter=metadata";
								// so document list can be used to display search results
								// search parameters
						 ...
						 *            in
						 * @return {YAHOO.widget.Record} Successful search
						 *         result or null
						 */
						_findRecordByParameter : function DL__findRecordByParameter(p_value, p_parameter) {
							var oRecordSet = this.widgets.dataTable.getRecordSet(), oRecord, record, i, j;
