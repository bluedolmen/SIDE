               ...
               <div id="${id}-createContent-menu" class="yuimenu">
                  <div class="bd">
                     ...
                     <#list createContent as content>
                        <#assign href>create-content?destination={nodeRef}&amp;itemId=${content.itemid}<#if (content.mimetype!"") != "">&amp;mimeType=${content.mimetype?html}</#if><#if (content.formid!"") != "">&amp;formId=${content.formid?html}</#if>&amp;isContainer=${content.isContainer!"false"}</#assign>
