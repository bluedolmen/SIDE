...
var DocumentList = {
	...
	getPreferences : function getPreferences() {
		...
		try {
			...
			if (result.status == 200 && result != "{}") {
				...
				// Populate the preferences object literal for easy look-up
				// later
	...
	getConfigValue : function getConfigValue(configFamily, configName, defaultValue) {
		var value = defaultValue, theConfig = config.scoped[configFamily][configName];
	...
	getReplicationUrlMappingJSON : function getReplicationUrlMappingJSON() {
		...
		try {
			var urlConfig, repositoryId, configs = config.scoped["Replication"]["share-urls"].getChildren("share-url");
	...
	getSortOptions : function getSortOptions() {
		// New Content
		var sortOptions = [], sortingConfig = config.scoped["DocumentLibrary"]["sorting"];

		if (sortingConfig !== null) {
			var configs = sortingConfig.getChildren(), configItem, sortLabel, sortValue, valueTokens;
	...
	isUserSiteManager : function isUserSiteManager() {
		...
		var userIsSiteManager = false, obj = null, site = args.site;
