...
(function() {
	...
	YAHOO.extend(SIDE.DocListTree, Alfresco.DocListTree, {
		 ...
		 * TODO @migration@ can be remove this rename _buildTreeNodeUrl_nodeChild -> _buildTreeNodeUrl
		 * @method onReady
		 */
			...
			this.fnLoadNodeData = function DLT_oR_fnLoadNodeData(node, fnLoadComplete) {
				...
				var callback = {
					success : function DLT_lND_success(oResponse) {
						...
						/**
						 * Execute the node's loadComplete callback method which
						 * comes in via the argument in the response object
						 */
		...
		onNodeClicked : function DLT_onNodeClicked(args) {
			...
			if (this.isFilterOwner && node == this.selectedNode) {
				...
				YAHOO.Bubbling.fire("changeFilter", {
					...
					filterId : "metadata",
					filterData : JSON.stringify(this._buildTreeNodeDocLibQuery(node))
				});
			}
