...
public class AlfrescoStructure {
	...
	public static NodeRef getContent(Map<String, String> commands, String[] parameters) throws DuplicateOutputContentException, MissingOutputContentException {
	...
	public static NodeRef getContentByUUID(String UUIDContent) {
		...
		if (nodeRefSet.length() >= 1) {
			...
			Object result = null;
			try {
				result = AlfrescoJavaScript.executeScript(executionContext, serviceRegistry, UUIDContent, Collections.<VariableAccess> emptyList(), ExtractDataFromContent.getCompanyHomeNodeRef(serviceRegistry));
			} catch (Exception e) {
				//Nothing to do
			}
	...
	private static PdfReader getReaderFromAlfrescoUUIDToPdf(String alfrescoUUID) throws IOException, NoPdfFileException {
		...
		if (node != null) {
			...
			//Trying to evaluate as alfresco javascript
			Object result = AlfrescoJavaScript.executeScript(executionContext, serviceRegistry, alfrescoUUID, Collections.<VariableAccess> emptyList(), ExtractDataFromContent.getCompanyHomeNodeRef(serviceRegistry));
			if (result instanceof JBPMNode) {
				JBPMNode jbpmnode = (JBPMNode) result;
				if (jbpmnode.getNodeRef() != null)
	...
	public static PdfStamper manageAlfrescoPDF(PdfReader reader, Map<String, String> commands) throws IOException, NoPdfFileException, MissingOutputPathForPDFException, DocumentException, MissingOverridePdfKeyException, FileExistsException, FileNotFoundException {
