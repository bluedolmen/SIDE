			...
			.extend(
					...
					Alfresco.component.Base,
					{
						...
						options : {
							...
							/**
							 * enable button to save current search in
							 * DataListItem
							 */
							...
							/**
							 * if true allow user to override existing
							 * savedSearch
							 */
						...
						onReady : function Search_onReady() {
							...
							if (this.options.enableSavedSearch
									&& this.options.siteId.length !== 0) {
								this.widgets.savedSearchButton = new YAHOO.widget.Button(
										{
											...
											type : "push",
											label : this
													.msg("button.save-search"),
											container : this.id
													+ "-right-buttons"
										});
								this.widgets.savedSearchButton.on("click",
										this.onSaveSearchClick,
										this.widgets.savedSearchButton, this);
							}
								...
								this.widgets.exportCSVButton = new YAHOO.widget.Button(
										{
											...
											type : "push",
											label : this
													.msg("button.search-export"),
											container : this.id
													+ "-right-buttons"
										});
								this.widgets.exportCSVButton.on("click",
										this.onExport2csv,
										this.widgets.exportCSVButton, this);
							}
						 ...
						 *            addListener method
						 */
						onSaveSearchClick : function Search_onSaveSearchClick(
								e, obj) {
							// open dialog to let user choose a name for the
							// saved
							// search and the visibility public/private default
							// private
							var saveName = prompt(this.msg("search.save"), "");

							if (saveName != null && saveName != "") {
								var queryString = this._buildSearchParams(
										this.searchRepository,
										this.searchAllSites, this.searchTerm,
										this.searchTag, this.searchSort);
								var url = Alfresco.constants.PROXY_URI
										+ 'side/repo/saveSearch/site/'
										+ Alfresco.constants.SITE
										+ '?dataType='
										+ this.options.savedSearchDataType
										+ '&saveName=' + saveName + "&"
										+ queryString;

								if (this.options.savedSearchVisibility
										.indexOf("private") != -1) {
									url += "&userName="
								...
								if (this.options.savedSearchAllowOverride) {
									url += "&overrideAllowed="
											+ this.options.savedSearchAllowOverride;
								}
								var responseConfig = {
						...
						onExport2csv : function Search_onExport2csv(e, args) {
							var queryString = this._buildSearchParams(
									this.searchRepository, this.searchAllSites,
									this.searchTerm, this.searchTag,
									this.searchSort, -1);

							var downloadUrl = Alfresco.constants.PROXY_URI
									+ "/com/bluexml/side/views.csv?viewName="
									+ this.options.exportViewName + "&"
									+ queryString;
							var displayName = "exported search.csv";
							if (YAHOO.env.ua.ie > 6) {
								// MSIE7 blocks the download and gets the wrong
								// URL in
								// the "manual download bar"
								Alfresco.util.PopupManager
										.displayPrompt({
											title : this
													.msg(
															"message.edit-offline.success",
															displayName),
											text : this
													.msg("message.edit-offline.success.ie7"),
											buttons : [
													{
														text : this
																.msg("button.download"),
								...
								// Kick off the download 3 seconds after the
						...
						_buildSearchParams : function Search__buildSearchParams(
								...
								searchTag, searchSort, maxResults) {
							var site = searchAllSites ? ""
									: this.options.siteId;
							var params = YAHOO.lang
									.substitute(
											"site={site}&term={term}&tag={tag}&maxResults={maxResults}&sort={sort}&query={query}&repo={repo}",
											{
												site : encodeURIComponent(site),
												repo : (searchRepository || this.options.searchQuery.length !== 0)
														.toString(), // always search entire repo with advanced query
												...
												maxResults : maxResults != null ? maxResults
														: this.options.maxSearchResults + 1
