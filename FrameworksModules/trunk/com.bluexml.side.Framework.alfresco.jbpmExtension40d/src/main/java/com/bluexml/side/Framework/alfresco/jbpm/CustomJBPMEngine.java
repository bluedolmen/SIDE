 ...
 * @author davidc
 */
public class CustomJBPMEngine extends AlfrescoBpmEngine implements WorkflowEngine
{
    private static Log logger = LogFactory.getLog(CustomJBPMEngine.class);
    ...
    * {@inheritDoc}
    ...
    protected CompiledProcessDefinition compileProcessDefinition(InputStream definitionStream, String mimetype)
    {
        ...
        else if (actualMimetype.equals(MimetypeMap.MIMETYPE_XML))
        {
            try
            {
                if (logger.isDebugEnabled()) logger.debug("Start deployment using BX CustomJBPMJpdlXmlReader");
