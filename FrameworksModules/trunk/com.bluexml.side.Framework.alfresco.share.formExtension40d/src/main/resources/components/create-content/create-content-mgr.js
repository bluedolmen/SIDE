...
(function() {
	...
	YAHOO.extend(Alfresco.CreateContentMgr, Alfresco.component.Base, {
		...
		options : {
			/**
			 * Is the created type expected to be a container? The manager needs
			 * to know whether the following page is document-details or
			 * folder-details.
			 * 
		...
		onFormContentReady : function CreateContentMgr_onFormContentReady(layer, args) {
			var formUI = args[1];
			// SIDE add-on test if this instance is the event recipient allow to
			// use many form dialog at same time
			// need to map the create-content-mgr component to the formComponent
			// id
		...
		onCreateContentSuccess : function CreateContentMgr_onCreateContentSuccess(response) {
			...
			if (response.json && response.json.persistedObject) {
				// Grab the new nodeRef and pass it on to _navigateForward() to
				// optionally use
		 ...
		 * @private
		 * @param nodeRef
		 *            {Alfresco.util.NodeRef} Optional: NodeRef of just-created
		 *            content item
		 */
		_navigateForward : function CreateContentMgr__navigateForward(nodeRef) {
			...
			if (YAHOO.lang.isObject(nodeRef)) {
				...
				// Nowhere sensible to go other than the default page unless
				// we're in a portal
