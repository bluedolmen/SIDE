         ...
         .extend(
               ...
               Alfresco.component.Base,
               {
                  ...
                  _setupDataTable : function Search_setupDataTable() {
                     ...
                     renderCellThumbnail = function Search_renderCellThumbnail(elCell, oRecord, oColumn, oData) {
                        ...
                        if (dataType === "document") {
                           ...
                           html = '<div class="action-overlay">' + '<a href="' + encodeURI(viewUrl) + '" target="_blank"><img title="' + $html(me.msg("label.viewinbrowser")) + '" src="'
											+ Alfresco.constants.URL_RESCONTEXT + 'components/search/images/view-in-browser-16.png" width="16" height="16"/></a>' + '<a href="'
                                 + encodeURI(viewUrl + "?a=true") + '" style="padding-left:4px" target="_blank"><img title="' + $html(me.msg("label.download")) + '" src="'
											+ Alfresco.constants.URL_RESCONTEXT + 'components/search/images/download-16.png" width="16" height="16"/></a>' + '</div>' + html;
                        }
                     ...
                     renderCellDescription = function Search_renderCellDescription(elCell, oRecord, oColumn, oData) {
                        ...
                        if (oRecord.getData("modifiedBy")) {
                           desc += ' ' + me.msg("message.modifiedby");
									desc += ' <a href="' + Alfresco.constants.URL_PAGECONTEXT + 'user/' + encodeURI(oRecord.getData("modifiedByUser")) + '/profile">'
                           ...
                           if (site) {
                              if (path === null || path === undefined) {
                                 path = "";
										...
										desc += '<div class="details">' + me.msg("message.infolderpath") + ': <a href="' + me._getBrowseUrlForFolderPath(path, site) + '">' + $html('/' + path)
												+ '</a></div>';
                           } else {
                              if (path) {
                  ...
                  _getBrowseUrlForRecord : function Search__getBrowseUrlForRecord(record) {
                     ...
                     var name = record.getData("name"), type = record.getData("type"), site = record.getData("site"), path = record.getData("path");
                  ...
                  refreshSearch : function Search_refreshSearch(args) {
                     ...
                     if (searchQuery.length !== 0) {
								...
								// apply it
								// other options such as tag, terms, all sites,
								// repo etc. are trumped
                        url += "&q=" + searchQuery;
                     } else {
                  ...
                  onSaveSearchClick : function Search_onSaveSearchClick(e, obj) {
                     // open dialog to let user choose a name for the saved
                     // search and the visibility public/private default private
                     ...
                     if (saveName != null && saveName != "") {
                        ...
                        var url = Alfresco.constants.PROXY_URI + 'side/repo/saveSearch/site/' + Alfresco.constants.SITE + '?dataType=' + this.options.savedSearchDataType + '&saveName=' + saveName
                              + "&" + queryString;
                  ...
                  onExport2csv : function Search_onExport2csv(e, args) {
                     ...
                     if (YAHOO.env.ua.ie > 6) {
                        // MSIE7 blocks the download and gets the wrong URL in
