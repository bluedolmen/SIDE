				...
				if (propName.indexOf("_") !== -1) {
					...
					if (propName.match("-range$") == "-range") {
						...
						if (propValue.length > 1) {
							...
							formQuery += (first ? '' : ' ' + operator + ' ') + escapeQName(propName) + ':"' + from + '".."' + to + '"';
						}
					} else if (propName.indexOf("cm:categories") != -1) {
						var useSubCats = false;
						// determines if the checkbox use sub categories was
						// clicked
						if (propName.indexOf("usesubcats") == -1) {
							...
							if (catQuery.length !== 0) {
								// surround category terms with brackets if
								// appropriate
						...
						if (propValue.indexOf(" " + orString + " ") != -1 || propValue.indexOf(" " + andString + " ") != -1 || propValue.indexOf(notString + " ") != -1 || propValue.indexOf("(") != -1
								|| propValue.indexOf(")") != -1) {
							// prop value contains sub query so interpret them
							// to
							// fts-alfresco query
							queryTerm = propValue;
							...
							for ( var c = 0; c < split.length; c++) {
								...
								if (item != andString && item != orString && item != notString) {
									...
									queryTerm = queryTerm.replace(regex, escapeQName(propName) + ':"' + tokenClean + '"');
								}
							}
							...
							if (propValue.indexOf(",") != -1 && propName != "cm:content" && m == null) {
								...
								for ( var tc = 0; tc < terms.length; tc++) {
									var cterm = terms[tc];
									var enumterm = escapeQName(propName) + ':"' + cterm + '"';
...
function getSearchDef(params) {
	...
	if (formData !== null && formData.length !== 0) {
		...
		if (formQuery.length !== 0 || ftsQuery.length !== 0 || params.repo) {
	...
	if (ftsQuery.length !== 0 || params.repo) {
