...
const
COMMENT_QNAMEPATH = "/fm:discussion/cm:Comments/";
const
QUERY_TEMPLATES = [ {
   field : "keywords",
   template : "%(cm:name cm:title cm:description ia:whatEvent ia:descriptionEvent lnk:title lnk:description TEXT)"
} ];
...
function getDataItem(siteId, containerId, pathParts, node) {
   ...
   if (node.type == "{http://www.alfresco.org/model/datalist/1.0}dataList") {
      ...
      item = {
         ...
         size : -1,
         displayName : node.name
      // unfortunately does not have a common
