...
function main()
{
    ...
    try
    {
        ...
        else
        {
            ...
            if (pathElements.length > 0)
            {
                ...
                if (logger.isLoggingEnabled()) logger.log("lastPathElement = " + lastPathElement);
            ...
            if (url.templateArgs.type == "node")
            {
                ...
                {
                    if (logger.isLoggingEnabled()) logger.log("ignoring types = " + argsFilterType);
                    ...
                    for (var pc = 0; pc < paths.length; pc++)
                    {
                        ...
                        if (pathItem.indexOf("://") != -1)
                        {
                            ...
                            if (groups != null)
                            {
                                ...
                                // special cases
                                // handling /../ example /cm:f1/cm:f2/../cm:toto/..//* -> /cm:f1//*
                                while (resolvedPathItem.indexOf("/..") != -1)
                                {
                                    var indexParent = resolvedPathItem.indexOf("/..");
                                    // logger.log("indexParent :" + indexParent);
                                    if (indexParent != -1)
                                    {
                                        // need to handle xpath parent axis
                                        // get the previous token position
                                        var s = resolvedPathItem.substring(0, indexParent);
                                        // logger.log("s :" + s);
                                        var indexParentToken = s.lastIndexOf("/");
                                        // logger.log("indexParentToken :" + indexParentToken);
                                        resolvedPathItem = resolvedPathItem.substring(0, indexParentToken) + resolvedPathItem.substring(indexParent + 3);
                                    }
                                }
                            }
                            else
    ...
    catch (e)
    {
        ...
        if (logger.isLoggingEnabled()) logger.log(msg);
...
function findUsers(searchTerm, maxResults, results, xpath)
{
    ...
    if (logger.isLoggingEnabled()) logger.log("user query = " + query);
    ...
    {
        if (logger.isLoggingEnabled()) logger.log("found user = " + node.userName);
...
function findGroups(searchTerm, maxResults, results, xpath)
{
    ...
    for each(var node in searchResults)
    {
        ...
        if (logger.isLoggingEnabled()) logger.log("group query = " + query);
...
function findDoclib(nodeRef)
{
    ...
    if (node !== null)
    {
        ...
        if (logger.isLoggingEnabled()) logger.log("siteName = " + siteName);
