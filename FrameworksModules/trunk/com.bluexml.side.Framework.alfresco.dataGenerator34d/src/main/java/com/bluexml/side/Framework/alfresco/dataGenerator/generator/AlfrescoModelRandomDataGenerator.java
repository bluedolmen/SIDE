...
public class AlfrescoModelRandomDataGenerator implements IRandomGenerator {
	...
	private static Map<ClassDefinition, Integer> indexType = new HashMap<ClassDefinition, Integer>();
	private static Map<ClassDefinition, Map<PropertyDefinition, Integer>> index = new HashMap<ClassDefinition, Map<PropertyDefinition, Integer>>();
	private static int numOfTypes;
	private static ClassDefinition typeRef;
	private Map<ClassDefinition, Map<PropertyDefinition, Integer>> numOfSame = new HashMap<ClassDefinition, Map<PropertyDefinition, Integer>>();
	private Map<ClassDefinition, Map<PropertyDefinition, Object>> sameData = new HashMap<ClassDefinition, Map<PropertyDefinition, Object>>();
	...
	private static class RandomMethods {
		...
		static ClassDefinition selectRandomlyType(Collection<ClassDefinition> types) {
			int randomIndex = randomGenerator.nextInt(types.size());
			return ((ArrayList<ClassDefinition>) types).get(randomIndex);
		}
	...
	public static Map<ClassDefinition, Integer> getIndexType() {
		return indexType;
	}

	public static void setIndexType(Map<ClassDefinition, Integer> indexType) {
	...
	public void setNativeGenerator(NativeAlfrescoModelRandomDataGenerator nativeGenerator) {
	...
	public static Map<ClassDefinition, Map<PropertyDefinition, Integer>> getIndex() {
		return index;
	}

	public static void setIndex(Map<ClassDefinition, Map<PropertyDefinition, Integer>> index) {
		AlfrescoModelRandomDataGenerator.index = index;
	}

	public static ClassDefinition getTypeRef() {
		return typeRef;
	}

	public static void setTypeRef(ClassDefinition typeRef) {
		AlfrescoModelRandomDataGenerator.typeRef = typeRef;
	}

	public Map<ClassDefinition, Map<PropertyDefinition, Integer>> getNumOfSame() {
		return numOfSame;
	}

	public void setNumOfSame(Map<ClassDefinition, Map<PropertyDefinition, Integer>> numOfSame) {
		this.numOfSame = numOfSame;
	}

	public Map<ClassDefinition, Map<PropertyDefinition, Object>> getSameData() {
		return sameData;
	}

	public void setSameData(Map<ClassDefinition, Map<PropertyDefinition, Object>> sameData) {
	...
	/**
	 * @param numberOfOutputArcs
	 *            the numberOfOutputArcs to set
	 */
	...
	/**
	 * @param alfrescoModelDatas
	 *            the alfrescoModelDatas to set
	 */
	...
	public boolean generateNodesInstances(IStructure structure) throws Exception {
		...
		Map<AssociationDefinition, Collection<ClassDefinition>> sourcesTypesOfCompositions = generatorServices.getSourcesTypesOfCompositions(createdTypes, compositions);
		Collection<ClassDefinition> deletedTargetsTypes = generatorServices.deleteTargetsTypesOfNotCreatedTargetsTypesOfCompositions(sourcesTypesOfCompositions, createdTypes);
	...
	private List<IArc> generateArcsInstancesCaseN1(Collection<INode> sourcesNodes, Collection<INode> targetsNodes, AssociationDefinition associationDefinition, Collection<AssociationDefinition> associations, Collection<IArc> generatedArcs) {
		List<IArc> arcsInstances = new ArrayList<IArc>();
	...
	private List<IArc> generateArcsInstancesCase1N(Collection<INode> sourcesNodes, Collection<INode> targetsNodes, AssociationDefinition associationDefinition, Collection<AssociationDefinition> associations, Collection<IArc> generatedArcs) {
		List<IArc> arcsInstances = new ArrayList<IArc>();
		while (!sourcesNodes.isEmpty()) {
			INode source = RandomMethods.selectRandomlyNode(sourcesNodes);
			int numberOfArcs = 0;
	...
	private List<IArc> generateArcsInstancesCase11(Collection<INode> sourcesNodes, Collection<INode> targetsNodes, AssociationDefinition associationDefinition, Collection<AssociationDefinition> associations, Collection<IArc> generatedArcs) {
		List<IArc> arcsInstances = new ArrayList<IArc>();
	...
	public Map<PropertyDefinition, Object> generateDatasProperties(ClassDefinition type, Collection<PropertyDefinition> properties) throws Exception {
	...
	private Map<PropertyDefinition, Object> generateDataForUnicityProperties(ClassDefinition type, Collection<PropertyDefinition> unicityProperties) throws Exception {
			...
			if (serializedData.size() > 0 && unicityProperties.size() > 0) {
				for (Serial dataProperty : dataProperties) {
					...
					for (Serial serializedDataProperty : serializedData) {
						if (dataProperty.getType().equals(serializedDataProperty.getType()) && dataProperty.getProperty().equals(serializedDataProperty.getProperty()) && dataProperty.getData().toString().equals(serializedDataProperty.getData().toString())) {
							contains = true;
						}
					}
					same &= contains;
	...
	private Object generateDatasProperty(PropertyDefinition propertyDefinition) throws Exception {
		...
		if (constraints.size() == 1) {//we suppose there is a one-to-one correspondence between property and enumeration
	...
	public Map<AspectDefinition, Map<PropertyDefinition, Object>> generateDataAspect(ClassDefinition type, Collection<AspectDefinition> aspects) throws Exception {
