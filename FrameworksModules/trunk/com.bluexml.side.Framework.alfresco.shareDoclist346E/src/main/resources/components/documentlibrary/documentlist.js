			...
			.augmentObject(
					Alfresco.DocumentList.prototype,
					{
						...
						options : {
							...
							/**
							 * Working mode: Site or Repository. Affects how
							 * actions operate, e.g. actvities are not posted in
							 * Repository mode.
							 * 
							...
							return function DL_renderCellStatus(elCell, oRecord, oColumn, oData) {
								...
								if (dataStatus.length > 0) {
									...
									for ( var i = 0, j = statuses.length; i < j; i++) {
										...
										// scope.msg()
										// function
										desc += '<div class="status"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/' + status + '-indicator-16.png" title="'
												+ tip + '" alt="' + status + '" /></div>';
									}
								}
								...
								if (status && status !== "") {
									...
									desc += '<div class="status"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/workflow-indicator-16.png" title="' + tip
											+ '" alt="' + tip + '" /></div>';
								}
								...
								if (scope.options.simpleView) {
									...
									if (type == "folder") {
										elCell.innerHTML = '<span class="folder-small">' + (isLink ? '<span class="link"></span>' : '')
												+ Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord) + '<img src="' + Alfresco.constants.URL_RESCONTEXT
										...
										elCell.innerHTML = '<span id="' + id + '" class="icon32">' + (isLink ? '<span class="link"></span>' : '')
												+ Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord) + '<img src="' + Alfresco.constants.URL_RESCONTEXT
												+ 'components/images/filetypes/' + Alfresco.util.getFileIcon(name) + '" alt="' + extn + '" title="' + $html(title) + '" /></a></span>';
									...
									if (type == "folder") {
										...
										elCell.innerHTML = '<span class="thumbnail">' + (isLink ? '<span class="link"></span>' : '')
												+ Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord) + '<img src="' + Alfresco.DocumentList.generateThumbnailUrl(oRecord) + '" alt="'
												+ extn + '" title="' + $html(title) + '" /></a></span>';
									}
								}
							};
						},
							...
							return function DL_renderCellDescription(elCell, oRecord, oColumn, oData) {
								...
								if (type == "folder") {
									...
									if (scope.options.simpleView) {
										 ...
										 * Simple View
										 */
										desc += '<div class="detail"><span class="item-simple"><em>' + scope.msg("details.modified.on") + '</em> '
												+ $date(record.modifiedOn, scope.msg("date-format.longDate")) + '</span>';
									...
									if (scope.options.simpleView) {
										...
										if (record.custom && record.custom.isWorkingCopy) {
											...
											if (customDesc == "") {
												...
												desc += '<span class="item"><em>' + scope.msg("details.editing-started.by") + '</em> ' + $userProfile(record.modifiedByUser, record.modifiedBy)
														+ '</span>';
						...
						fnRenderCellActions : function DL_fnRenderCellActions() {
							...
							return function DL_renderCellActions(elCell, oRecord, oColumn, oData) {
								...
								/**
								 * Configure the Online Edit URL if enabled for
								 * this mimetype
								 */
								if (scope.doclistMetadata.onlineEditing && (record.mimetype in scope.onlineEditMimetypes)) {
									var loc = record.location, uri = scope.options.vtiServer.host + ":" + scope.options.vtiServer.port + "/"
											+ $combine("alfresco", loc.site, loc.container, loc.path, loc.file);
						...
						/**
						 * parse the this.options.sorting String
						 * return {
						 * 		    key : the sorted field, cm:name ...
						 * 	 	    ascendant : the sort order, true|false 
						 *        }
						 */
						 ...
						 *            node
						 * @param siteId
						 *            {string} Optional siteId override for
						...
						_updateDocList : function DL__updateDocList(p_obj) {
							...
							var successFilter = YAHOO.lang.merge({}, p_obj.filter !== undefined ? p_obj.filter : this.currentFilter), successPage = p_obj.page !== undefined ? p_obj.page
									: this.currentPage, loadingMessage = null, timerShowLoadingMessage = null, me = this, params = {
