...
(function() {
    ...
    * Preferences
    */
   var PREFERENCES_DOCLIST = "org.alfresco.share.documentList", PREF_SHOW_FOLDERS = PREFERENCES_DOCLIST + ".showFolders", PREF_SIMPLE_VIEW = PREFERENCES_DOCLIST + ".simpleView";
   ...
   Alfresco.DocumentList.generateFilterMarkup = function DL_generateFilterMarkup(filter) {
      var filterObj = Alfresco.util.cleanBubblingObject(filter), markup = YAHOO.lang.substitute("{filterOwner}|{filterId}|{filterData}|{filterDisplay}", filterObj, function(p_key, p_value, p_meta) {
    ...
    * @param oRecord
    *           {YAHOO.widget.Record} File record
    * @return {string} Mark-up for use in node attribute
    * 
    * <pre>
   ...
   Alfresco.DocumentList.generateFileFolderLinkMarkup = function DL_generateFileFolderLinkMarkup(scope, oRecord) {
      var record = oRecord.getData(), html;
   ...
   Alfresco.DocumentList.generateThumbnailUrl = function DL_generateThumbnailUrl(oRecord) {
      var record = oRecord.getData(), nodeRef = new Alfresco.util.NodeRef(record.isLink ? record.linkedNodeRef : record.nodeRef);
   ...
   Alfresco.DocumentList.generateFavourite = function DL_generateFavourite(scope, record) {
      var id = scope.id + "-fav-" + record.getId(), type = record.getData("isFolder") ? "folder" : "document", isFavourite = record.getData("isFavourite");

      return '<a id="' + id + '" class="favourite-' + type + (isFavourite ? ' enabled' : '') + '" title="' + scope.msg("tip.favourite-" + type + "." + (isFavourite ? 'remove' : 'add'))
            + '">&nbsp;</a>';
   };
         ...
         .augmentObject(
               Alfresco.DocumentList.prototype,
               {
                  ...
                  options : {
                     ...
                     /**
                      * Flag indicating whether the list shows a detailed view
                      * or a simple one.
                      * 
                      * @property simpleView
                      * @type boolean
                     ...
                     /**
                      * Working mode: Site or Repository. Affects how actions
                      * operate, e.g. actvities are not posted in Repository
                      * mode.
                      * 
                     ...
                     /**
                      * ContainerId representing root container. Not used in
                     ...
                     /**
                      * Initial page to show on load (otherwise taken from URL
                      * hash).
                      * 
                     ...
                     /**
                      * Delay before showing "loading" message for slow data
                      * requests
                      * 
                     ...
                     /**
                      * Valid inline edit mimetypes Currently allowed are plain
                      * text, HTML and XML only
                      * 
                     ...
                     /**
                      * SIDE sorting store the sort query
                      */
                     sorting : ''
                  },
                  ...
                  /**
                   * Deferred actions menu element when showing "More Actions"
                   * pop-up.
                   * 
                  ...
                  onReady : function DL_onReady() {
                     ...
                     this.widgets.previewTooltip.contextTriggerEvent.subscribe(function(type, args) {
                        var context = args[0], record = me.widgets.dataTable.getRecord(context.id);
                  ...
                  fnRenderCellSelected : function DL_fnRenderCellSelected() {
                     ...
                     return function DL_renderCellSelected(elCell, oRecord, oColumn, oData) {
                        ...
                        elCell.innerHTML = '<input id="checkbox-' + oRecord.getId() + '" type="checkbox" name="fileChecked" value="' + oData + '"'
                              + (scope.selectedFiles[oData] ? ' checked="checked">' : '>');
                     };
                  },
                  ...
                  fnRenderCellStatus : function DL_fnRenderCellStatus() {
                     ...
                     return function DL_renderCellStatus(elCell, oRecord, oColumn, oData) {
                        ...
                        var record = oRecord.getData(), dataStatus = record.status, status, tip = "", desc = "";
                        ...
                        if (dataStatus.length > 0) {
                           ...
                           for ( var i = 0, j = statuses.length; i < j; i++) {
                              ...
                              tip = Alfresco.util.message(i18n, scope.name, meta.split("|")); // Note:
                              // deliberate
                              // bypass
                              // of
                              // scope.msg()
                              // function
                              desc += '<div class="status"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/' + status + '-indicator-16.png" title="' + tip
                                    + '" alt="' + status + '" /></div>';
                           }
                        }
                        ...
                        if (status && status !== "") {
                           ...
                           desc += '<div class="status"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/workflow-indicator-16.png" title="' + tip + '" alt="'
                                 + tip + '" /></div>';
                        }
                  ...
                  fnRenderCellThumbnail : function DL_fnRenderCellThumbnail() {
                     ...
                     return function DL_renderCellThumbnail(elCell, oRecord, oColumn, oData) {
                        var record = oRecord.getData(), name = record.fileName, title = record.title, type = record.type, isLink = record.isLink, extn = name.substring(name.lastIndexOf("."));
                        ...
                        if (scope.options.simpleView) {
                           ...
                           if (type == "folder") {
                              elCell.innerHTML = '<span class="folder-small">' + (isLink ? '<span class="link"></span>' : '') + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord)
                                    + '<img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/folder-32.png" /></a>';
                           } else {
                              var id = scope.id + '-preview-' + oRecord.getId();
                              elCell.innerHTML = '<span id="' + id + '" class="icon32">' + (isLink ? '<span class="link"></span>' : '')
                                    + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord) + '<img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/images/filetypes/'
                                    + Alfresco.util.getFileIcon(name) + '" alt="' + extn + '" title="' + $html(title) + '" /></a></span>';
                           ...
                           if (type == "folder") {
                              elCell.innerHTML = '<span class="folder">' + (isLink ? '<span class="link"></span>' : '') + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord)
                                    + '<img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/folder-48.png" /></a>';
                           } else {
                              elCell.innerHTML = '<span class="thumbnail">' + (isLink ? '<span class="link"></span>' : '') + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord)
                                    + '<img src="' + Alfresco.DocumentList.generateThumbnailUrl(oRecord) + '" alt="' + extn + '" title="' + $html(title) + '" /></a></span>';
                           }
                        }
                     };
                  },
                  ...
                  fnRenderCellDescription : function DL_fnRenderCellDescription() {
                     ...
                     return function DL_renderCellDescription(elCell, oRecord, oColumn, oData) {
                        ...
                        var record = oRecord.getData(), type = record.type, isLink = record.isLink, title = "", description = record.description || scope.msg("details.description.none");
                        ...
                        if (type == "folder") {
                           ...
                           if (scope.options.simpleView) {
                               ...
                               * Simple View
                               */
                              desc += '<div class="detail"><span class="item-simple"><em>' + scope.msg("details.modified.on") + '</em> ' + $date(record.modifiedOn, scope.msg("date-format.longDate"))
                                    + '</span>';
                              ...
                              if (record.custom.googleDocUrl && record.custom.googleDocUrl !== "") {
                                 if (record.lockedByUser === Alfresco.constants.USERNAME) {
                                    desc += '<div class="info-banner">'
                                          + scope.msg("details.banner.google-docs-owner", '<a href="' + record.custom.googleDocUrl + '" target="_blank">'
                                                + scope.msg("details.banner.google-docs.link") + '</a>') + '</div>';
                                 } else {
                                    desc += '<div class="info-banner">'
                                          + scope.msg("details.banner.google-docs-locked", lockedByLink, '<a href="' + record.custom.googleDocUrl + '" target="_blank">'
                                                + scope.msg("details.banner.google-docs.link") + '</a>') + '</div>';
                                 }
                              }
                           ...
                           desc += '<h3 class="filename">' + Alfresco.DocumentList.generateFavourite(scope, oRecord) + '<span id="' + scope.id + '-preview-' + oRecord.getId() + '">'
                                 + Alfresco.DocumentList.generateFileFolderLinkMarkup(scope, oRecord);
                  ...
                  fnRenderCellActions : function DL_fnRenderCellActions() {
                     ...
                     return function DL_renderCellActions(elCell, oRecord, oColumn, oData) {
                        ...
                        /**
                         * Configure the Online Edit URL if enabled for this
                         * mimetype
                         */
                        if (scope.doclistMetadata.onlineEditing && (record.mimetype in scope.onlineEditMimetypes)) {
                           var loc = record.location, uri = scope.options.vtiServer.host + ":" + scope.options.vtiServer.port + "/" + $combine("alfresco", loc.site, loc.container, loc.path, loc.file);

                           if (!(/^(http|https):\/\//).test(uri)) {
                              // VTI server now supports HTTPS
                              // directly
                              // http://issues.alfresco.com/jira/browse/DOC-227
                  ...
                  _setupDataSource : function DL__setupDataSource() {
                     ...
                     this.widgets.dataSource.doBeforeCallback = function DL_doBeforeCallback(oRequest, oFullResponse, oParsedResponse) {
                        ...
                        // Reset onlineEdit flag if correct conditions
                        // not met
                        ...
                        // Update "Empty" message to reflect subfolders
                        ...
                        if (itemCounts.documents === 0 && itemCounts.folders > 0 && !me.options.showFolders) {
                           var showFoldersLink = '<span class="show-folders-link theme-color-1" onclick="Alfresco.util.ComponentManager.get(\'' + me.id + '\').onShowFolders();">'
                                 + me.msg("message.empty.subfolders.link") + '</span>';
                   ...
                   * including instantiation of YUI widgets and event listener
                   * binding.
                   * 
                  ...
                  getSelectedFiles : function DL_getSelectedFiles() {
                     var files = [], recordSet = this.widgets.dataTable.getRecordSet(), record;
                  ...
                  selectFiles : function DL_selectFiles(p_selectType) {
                     var recordSet = this.widgets.dataTable.getRecordSet(), checks = YAHOO.util.Selector.query('input[type="checkbox"]', this.widgets.dataTable.getTbodyEl()), len = checks.length, record, i, fnCheck, typeMap;
                   ...
                   * @param aArgs
                   *           {array} Arguments array, [0] = DomEvent, [1] =
                   *           EventTarget
                   * @param p_obj
                  ...
                  /**
                   * parse the this.options.sorting String return { key : the
                   * sorted field, cm:name ... ascendant : the sort order,
                   * true|false }
                   */
                  parseSortingString : function _parseSortingString(string) {
                     var sorting = {
                        key : string,
                        ascendant : true
                     };
                     ...
                     if (sepIndex != -1) {
                        var sortingObject = string.split("|");
                        sorting.key = sortingObject[0];
                        sorting.ascendant = sortingObject[1] == "true" ? true : false;
                     }
                     return sorting;
                  },

                  /**
                   * sorting doclist SIDE extension
                   * 
                   */
                  onSort : function DL_onSort(sType, aArgs, p_obj) {
                     var path = p_obj.currentPath;
                     var index = aArgs[1].index;
                     // get className of the selected element
                     var target = aArgs[0].originalTarget;
                     var className = "";
                     if (target.className.indexOf("sort-") != -1) {
                        className = target.className;
                     } else {
                        // may be not the right dom element
                        className = target.childNodes[0].className;
                     }

                     var key = className.replace("sort-", "");
                     var newSorting = this.parseSortingString(key);
                     var oldSorting = this.parseSortingString(this.options.sorting);

                     if (oldSorting.key == newSorting.key) {
                        // already selected, so change sort order
                        newSorting.ascendant = !oldSorting.ascendant;
                     }

                     this.options.sorting = newSorting.key + "|" + newSorting.ascendant;
                  ...
                  onEventHighlightRow : function DL_onEventHighlightRow(oArgs) {
                     ...
                     if (elActions && elActions.firstChild === null) {
                        ...
                        var record = this.widgets.dataTable.getRecord(oArgs.target.id), actionSet = record.getData("actionSet");
                   ...
                   * @param recordData
                   *           {object} Object literal representing the node
                   * @param siteId
                   *           {string} Optional siteId override for site-based
                   *           locations
                  ...
                  getActionUrls : function DL_getActionUrls(recordData, siteId) {
                     var nodeRef = recordData.isLink ? recordData.linkedNodeRef : recordData.nodeRef, nodeRefUri = new Alfresco.util.NodeRef(nodeRef).uri, contentUrl = recordData.contentUrl, custom = recordData.custom, siteObj = YAHOO.lang
                           .isString(siteId) ? {
                        site : siteId
                     } : null, fnPageURL = Alfresco.util.bind(function(page) {
                  ...
                  /**
                   * BUBBLING LIBRARY EVENT HANDLERS FOR ACTIONS Disconnected
                   * event handlers for action event notification
                   */
                  ...
                  onActionShowMore : function DL_onActionShowMore(asset, elMore) {
                     ...
                     // Initial after-click hide timer - 5x the mouseOut
                     // timer delay
                   ...
                   * @param asset
                   *           {object} Object literal representing file or
                  ...
                  onActionEditOffline : function DL_onActionEditOffline(asset) {
                     if (!this.state.actionEditOfflineActive) {
                        ...
                        var nodeRef = new Alfresco.util.NodeRef(asset.nodeRef), displayName = asset.displayName;
                  ...
                  onActionCheckoutToGoogleDocs : function DL_onActionCheckoutToGoogleDocs(asset) {
                     var displayName = asset.displayName, nodeRef = new Alfresco.util.NodeRef(asset.nodeRef), path = asset.location.path, fileName = asset.fileName;
                   ...
                   * @param asset
                   *           {object} Object literal representing the file to
                   *           be actioned upon
                   */
                  onActionCheckinFromGoogleDocs : function DL_onActionCheckinFromGoogleDocs(asset) {
                     var displayName = asset.displayName, nodeRef = new Alfresco.util.NodeRef(asset.nodeRef), originalNodeRef = new Alfresco.util.NodeRef(asset.custom.workingCopyOriginal), path = asset.location.path;
                   ...
                   * (including the DocList itself on loading) wants to scroll
                   * to and highlight a file
                   * 
                  ...
                  /**
                   * Deactivate Dynamic Controls event handler Only deactivates
                   * specifically defined controls.
                   * 
                  ...
                  /**
                   * Activate Dynamic Controls event handler (Re-)Activates
                   * controls taking part in dynamic deactivation
                   * 
                   ...
                   * @param row
                   *           {HTMLElement} DOM reference to a TR element (or
                   *           child thereof)
                   */
                  ...
                  _favouriteHandler : function DL__favouriteHandler(row, prefKey) {
                     var record = this.widgets.dataTable.getRecord(row), file = record.getData(), nodeRef = file.nodeRef;
                   ...
                   * @param p_obj.page
                   *           {string} Optional page to navigate to (defaults
                   *           to this.currentPage)
                   */
                  _updateDocList : function DL__updateDocList(p_obj) {
                     p_obj = p_obj || {};
                     Alfresco.logger.debug("DL__updateDocList: ", p_obj.filter, p_obj.page);
                     var successFilter = YAHOO.lang.merge({}, p_obj.filter !== undefined ? p_obj.filter : this.currentFilter), successPage = p_obj.page !== undefined ? p_obj.page : this.currentPage, loadingMessage = null, timerShowLoadingMessage = null, me = this, params = {
                   ...
                   * @param p_obj.type
                   *           {string} Filetype to filter: "all", "documents",
                   *           "folders"
                   * @param p_obj.site
                   *           {string} Current site
                  ...
                  _buildDocListParams : function DL__buildDocListParams(p_obj) {
                     ...
                     if (this.options.sorting != '') {
                        params += "&sorting=" + encodeURIComponent(this.options.sorting);
                     }
                     return params;
                  },

                  /**
                   * Searches the current recordSet for a record with the given
                   * parameter value
                   * 
                   * @private
