 ...
 * 
 * @namespace Alfea.component
 * @class Alfea.component.ActivityStats
 */

if (typeof Alfea == "undefined" || !Alfea) {
	var Alfea = {};
}
Alfea.component = Alfea.component || {};
...
( function() {
	 ...
	 * @return {Alfea.component.ActivityStats} The new component instance
	 * @constructor
	 */
	Alfea.component.ActivityStats = function ActivityStats_constructor(htmlId) {
		Alfea.component.ActivityStats.superclass.constructor.call(this,
				"Alfea.component.ActivityStats", htmlId, [ "button",
	...
	YAHOO.extend(
	Alfea.component.ActivityStats,
	Alfresco.component.Base,
	{
		/**
		 * ActivityStats module instance.
		 *
		 * @property createSite
		 * @type Alfea.component.ActivityStats
		 */
		activityStats : null,
		 ...
		 * @method onReady
		 */
		onReady : function ActivityStats_onReady() {
			var me = this;
		 ...
		 */
		onRefresh : function ActivityStats_onRefresh(p_data) {
		 ...
		 * @method loadActivityStats
		 */
		 loadActivityStats : function ActivityStats_loadActivityStats() {
			this.widgets.feedbackMessage = Alfresco.util.PopupManager.displayMessage( {
				text : this.msg("activity-stats.message.refreshing"),
				spanClass : "wait",
				displayTime : 0
			});
			...
			{		
				...
				url: Alfresco.constants.PROXY_URI + "alfea/activity-stats/activity-stats",
				...
				//successMessage: this.msg("activity-stats.message.success"),
				failureCallback: {
					fn: this.onFailure,
					scope: this
				},
				failureMessage: this.msg("activity-stats.message.failure"),
				execScripts: true
			});
		},
		 ...
		 * @param p_data {object} call result data
		 */
		onSuccess : function ActivityStats_onSuccess(p_data) {
		 ...
		 * @param p_data {object} call result data
		 */
		onFailure : function ActivityStats_onFailure(p_data) {
			this.widgets.feedbackMessage.destroy();
		},
		 ...
		 */
		getPeriodValue : function ActivityStats_getPeriodValue() {
		 ...
		 * @param divId
		 */
		 initMultiSelect : function ActivityStats_initMultiSelect(divId) {
		 ...
		 */
		 getPersonValue : function ActivityStats_getPersonValue() {
		 ...
		 */
		getTypesValues : function ActivityStats_getTypesValues() {
		 ...
		 */
		getSitesValues : function ActivityStats_getSitesValues() {
		 ...
		 * @param divId 
		 */
		 getMultipleValues : function ActivityStats_getMultipleValues(divId) {
		 ...
		 */
		populateCharts : function ActivityStats_populateCharts(charts) {
			if(charts.length > 0){
			    ...
			    for(var i=0 ; i < charts.length ; i++){
			    	...
			    	if(charts[i].name == "all"){
			    		data.addColumn('number', this.msg("activity-stats.sites.all"));
			    	}
			    	else{
				...
				if(activitiesSum == 0){
					this.widgets.feedbackMessage.destroy();
					this.widgets.feedbackMessage = Alfresco.util.PopupManager.displayMessage( {
						text : this.msg("activity-stats.message.no-activity"),
						spanClass : "alert",
						displayTime : 3
					});
					return;
				...
				this.widgets.feedbackMessage = Alfresco.util.PopupManager.displayMessage( {
					text : this.msg("activity-stats.message.success"),
					spanClass : "alert",
					displayTime : 3
				});	
				return;
			...
			this.widgets.feedbackMessage = Alfresco.util.PopupManager.displayMessage( {
				text : this.msg("activity-stats.message.failed"),
				spanClass : "alert",
				displayTime : 3
			});
		}, 
		 ...
		 * @param charts loaded charts data 
		 */
		convertDateToLabel : function ActivityStats_convertDateToLabel(dateStr) {
	...
	var dummyInstance = new Alfea.component.ActivityStats("null");
