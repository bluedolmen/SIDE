...
(function()
{
   ...
   YAHOO.extend(Alfresco.module.SimpleDialog, Alfresco.component.Base,
   {
      ...
      {
    	  Alfresco.logger.debug("onHideEvent this :"+this.id);
      ...
      {
    	  Alfresco.logger.debug("onTemplateLoaded this "+this.id);  
      ...
      onBeforeFormRuntimeInit: function AmSD_onBeforeFormRuntimeInit(layer, args)
      {
         ...
         Alfresco.logger.debug("onBeforeFormRuntimeInit currentId :" + this.id);
         Alfresco.logger.debug("onBeforeFormRuntimeInit event from :" + formUI.formId);
         //SIDE add-on test if this instance is the event recipient allow to use many form dialog at same time
         if (this.id + "-form" == formUI.id) {
        	 Alfresco.logger.debug("onBeforeFormRuntimeInit good event continue ...");
      ...
      {
    	  Alfresco.logger.debug("onCancel this :"+this.id);
         this.dialog.hide();
      },
      ...
      {
    	  Alfresco.logger.debug("onSuccess this :"+this.id);
         this.dialog.hide();
      ...
      {
    	  Alfresco.logger.debug("onFailure this :"+this.id);
         // Make sure ok button is in the correct state if dialog is reused
         if (this.isFormOwner)
         {
            this.widgets.okButton.set("disabled", false);
