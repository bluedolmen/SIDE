...
<#if field.control.params.search??>
	<#assign modeSearch=field.control.params.search == "true">
<#else>
	<#assign modeSearch=false>
</#if>
...
<#if fieldValue?string != "" && !modeSearch>
...
<div class="form-field">
   ...
   <#else>
      ...
      <input id="${fieldHtmlId}" type="hidden" name="${field.name}" value="<#if field.mandatory>${fieldValue?string}</#if>" />
      <#if field.control.params.options?? && field.control.params.options != "">
         <select id="${fieldHtmlId}-entry" name="-" multiple="multiple" size="${size}" tabindex="0"
               ...
               <#if !field.mandatory><option value=""<#if values?size == 0> selected="selected"</#if>></option></#if>
               <#list field.control.params.options?split(optionSeparator) as nameValue>
