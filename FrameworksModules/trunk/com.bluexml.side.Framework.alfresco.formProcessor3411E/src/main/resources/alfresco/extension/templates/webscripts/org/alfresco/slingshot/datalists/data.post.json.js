...
function getData()
{
   ...
   if (json.has("fields"))
   {
      ...
      {
    	  // SIDE PATCH
    	  // use new String constructor to have replace work as javacript method instead of java one
    	  // so only replace the first occurrence SIDE model use underscore in qnames
         fields.push(new String(jsonFields.get(count)).replace("_", ":"));
      }
   }
